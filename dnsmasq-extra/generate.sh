...
curl_githubusercontent https://raw.githubusercontent.com/privacy-protection-tools/anti-AD/refs/heads/master/discretion/anv.txt | grep -v '^#' | tee -a adblock >/dev/null
...
# whitelist
curl_githubusercontent https://raw.githubusercontent.com/217heidai/adblockfilters/e32a8f6b291d2833a2ca8accb852cafd48960ddf/rules/white.txt | grep -v '^#' | grep -E '^[a-z\-\.]*$' | tee adblock.white >/dev/null
cat adblock.white | while read it; do
	sed "/^${it}$/d" -i adblock
done
rm -f adblock.white
sed 's+\.$++g' -i adblock
